...
class MemoListViewModel: CommonViewModel {
    ...
    func performUpdate(memo: Memo) -> Action<String, Void> {
        return Action { input in
            // 입력 값으로 메모를 업데이트 하도록 설정합니다.
            // - 반환형을 보면 출력타입이 리턴형으로 Void를 선언하고 있습니다.
            // - .map { _ in } 을 추가하면 Observable<Void> 형으로 반환할 수 있습니다.
            self.storage.update(memo: memo, content: input).map { _ in }
        }
    }
    ...
    func performCancel(memo: Memo) -> CocoaAction {
        return Action {
            // 메모가 생성 된 후, 저장하지 않고 취소시에 별도의 처리를 하지 않으면 불필요한 내용이 테이블뷰에 저장될 수 있어 이에 대한 처리가 필요합니다.
            ...
            // Observable<Memo>.map { _ in }
            self.storage.delete(memo: memo).map { _ in }
        }
    }

    /// 메모를 방출하는 Observable이 반환됩니다.
        ...
        return CocoaAction { _ in
            ...
            self.storage.createMemo(content: "") // 이어서 flatMap 연산자를 호출하고 클로저에서 화면전환을 처리합니다.
                .flatMap { memo -> Observable<Void> in
                    ...
                    // title은 바로 문자열을 전달하면 되고, SceneCoordinator와 storage에 대한 의존성은 현재 ViewModel에 있는 속성으로 바로 주입할 수 있습니다.
                    ...
                    // 마지막으로 sceneCoordinator에서 composeScene으로 modal방식으로 전환합니다.
                    // - transition 메서드는 completable을 반환합니다. 이를 void 형식의 Observable<Void>로 변환 후 리턴합니다.
