...
class MemoComposeViewController: UIViewController, ViewModelBindableType {
    ...
    func bindViewModel() {
        // 6-9) 먼저, navigation title 을 바인딩하겠습니다.(Driver<String>)
        viewModel.title
            .drive(navigationItem.rx.title)
            .disposed(by: rx.disposeBag)

        // 6-9) 이어서 initialText를 textView에 바인딩하겠습니다. (Driver<String>)
        viewModel.initialText
            .drive(contentTextView.rx.text)
            .disposed(by: rx.disposeBag)

        // 6-10) 취소버튼은 cancelAction과 바인딩하겠습니다.
        // action 패턴으로 action 구현시는 아래와 같이 action 속성(CocoaAction)에 저장하는 방식으로 바인딩해서 처리할 수 있습니다.
        cancelButton.rx.action = viewModel.cancelAction

        // 6-10) 저장버튼도 구현합니다. doubleTap(중복 탭)을 막기위해서 throttle연산자를 활용합니다.
        // - 0.5초 마다 한번씩만 탭이 되도록 설정합니다.
        saveButton.rx.tap
            .throttle(.milliseconds(500), scheduler: MainScheduler.instance)
            .withLatestFrom(contentTextView.rx.text.orEmpty) // withLatestFrom 연산자로 텍스트뷰의 텍스트를 방출할 수 있습니다.
            .bind(to: viewModel.saveAction.inputs) // 이어서 방출된 텍스트를 saveAction과 바인딩 하겠습니다. (Observable<String>)
            .disposed(by: rx.disposeBag)
    ...
    // 6-08) viewWillAppear가 호출될 때 키보드를 띄우기 위해 contentTextView를 firstResponder로 설정하겠습니다.
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
        contentTextView.becomeFirstResponder()
    }

    // 6-09) viewWillDisappear 호출시에는 키보드를 내리기 위해 firstResponder를 해제하겠습니다.
